...
{ 
    ...
    const {id}=useParams();
    useEffect(()=>{
        Axios.get("https://crud1-yy21.onrender.com/students/update-student/"+id)
        .then((res)=>{
            if(res.status===200){
    ...
    const handleSubmit = () =>{
        const data = {name: 
            newData[0],email:newData[1],rollNo:newData[2]}
            Axios.put("https://crud1-yy21.onrender.com/students/update-student/"+ id, data)
        .then((res)=>{
            if(res.status === 200){
